...
public class AddNewMessageThread implements Runnable {
    ...
    public void run() {
        try {
            ...
            long messages_last_UID_mail = imap_folder.LASTUID; // imap_folder.getUID(imap_folder.getMessage(imap_folder.getMessageCount()));
            int  messages_last_UID_db = db.getCountMessages(emailAccount.getUser().getUser_id(), myFolder.getFolder_name());
            ...
            if (messages_count_mail > 0) {
                ...
                if ((messages_count_db > 0) || (messages_last_UID_mail == messages_last_UID_db)) {
